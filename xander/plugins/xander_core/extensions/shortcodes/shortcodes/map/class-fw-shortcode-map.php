...
class FW_Shortcode_Map extends FW_Shortcode {
		...
		if (empty($this->data)) {
			$this->data = apply_filters('fw_shortcode_map_provider', array(
				'custom' => array(
					...
					'label'      => __('Custom','xander'),
					'options'    => array(
						'locations' => array(
							'label' => __('Locations', 'xander'),
							'popup-title' => __('Add/Edit Location', 'xander'),
							'type' => 'addable-popup',
							'desc' => false,
							'template' => '{{  if (location.location !== "") {  print(location.location)} else { print("' . __('Note: Please set location', 'xander') . '")} }}',
							'popup-options' => array(
								'location' => array(
									'type' => 'map',
									'label' =>__('Location','xander'),
								),
								'title' => array(
									'type' => 'text',
									'label' => __('Location Title', 'xander'),
									'desc' => __('Set location title', 'xander'),
								),
								'description' => array(
									'type'  => 'textarea',
									'label' => __('Location Description', 'xander'),
									'desc'  => __('Set location description', 'xander')
								),
								'url' => array(
									'type'  => 'text',
									'label' => __('Location Url', 'xander'),
									'desc'  => __('Set page url (Ex: http://example.com)', 'xander'),
								),
								'thumb' => array(
									'label'       => __('Location Image', 'xander'),
									'desc'        => __('Add location image', 'xander'),
									'type'        => 'upload',
								)
							)
						)
	...
	protected function _render($atts, $content = null, $tag = '')
	{
		if (!isset($atts['data_provider']['population_method'])) {
			trigger_error(
				__('No location provider specified for map shortcode', 'xander')
			);
			return '<b>' . __( 'Map Placeholder', 'xander' ) . '</b>';
		}
		...
		if (!isset($this->data[$provider])) {
			return '<!-- WARNING: '
			       . sprintf(__('Unknown location provider "%s" specified for map shortcode', 'xander'), $provider)
			       . ' -->';
		}
